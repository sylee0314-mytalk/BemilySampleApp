...
import com.olivia.architecturetemplate.util.BemilyGifError;
...
public abstract class GifEncoderAsyncTask extends AsyncTask<Void, Double, Boolean> {
    ...
    protected BemilyGifException exception;
    ...
    protected Boolean doInBackground(Void... params) {
        try {
            ...
            if (!encodeFrames(encoder)) {
                throw new BemilyGifException(BemilyGifError.FAILED_TO_CREATE_GIF_NO_FRAME);
            }

            encoder.finish();
            IOUtils.closeQuietly(out);
        } catch (BemilyGifException mex) {
            this.exception = mex;
            return false;
        } catch (Exception ex) {
            this.exception = new BemilyGifException(BemilyGifError.FAILED_TO_CREATE_GIF);
